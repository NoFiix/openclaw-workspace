...
export async function handler(ctx) {
  ...
  const { events: posts,    nextCursor: nextPostCursor    } = ctx.bus.readSince("trading.raw.social.post",   postCursor,    3);
  const { events: articles, nextCursor: nextArticleCursor } = ctx.bus.readSince("trading.raw.news.article", articleCursor, 5);
  ...
  // Mise à jour curseurs — uniquement jusqu'au dernier article traité
  ctx.state.cursors = {
    ...ctx.state.cursors,
    social_posts:   items.length > 0 ? nextPostCursor    : postCursor,
    news_articles:  items.length > 0 ? nextArticleCursor : articleCursor,
  };
